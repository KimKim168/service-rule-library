                    ...
                    <TableBody className="table-body rounded-md">
                        {tableData?.data?.map((item: any, index: number) => (
                            <TableRow
                                className="table-row"
                                key={item.id}
                                onDoubleClick={() => router.visit(`/admin/video-categories?category_code=${item.code}`)}
                            >
                                ...
                                <TableCellActions>
                                    {item.deleted_at ? (
                                        <RecoverItem
                                            ...
                                            recoverPath={`/admin/video-categories/${item.id}/recover`}
                                            permission="user update"
                                        />
                                    ) : (
                                        <>
                                            {/* Edit Dialog */}
                                            <EditItemButton url={`/admin/video-categories/${item.id}/edit`} permission="video_category update" />

                                            {/* View Dialog */}
                                            <ViewItemButton url={`/admin/video-categories/${item.id}`} permission="video_category view" />

                                            {/* Delete Item */}
                                            <DeleteItemButton deletePath="/admin/video-categories/" id={item.id} permission="video_category delete" />
                                        </>
                                    )}
                                </TableCellActions>
                                <TableCell>
                                    <TableCellAvatar
                                        ...
                                        alt={item.name}
                                        image={`/assets/images/video_categories/thumb/${item.image}`}
                                    />
                                </TableCell>
